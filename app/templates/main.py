...
def tela_gestao_tecnicos(request: Request):
    ...
    # Vinculação automática DESATIVADA (2026-07): o vínculo técnico-supervisor
    # agora é sempre manual, feito pelo coordenador em
    # /coordenador/cadastros/associar. Antes, essa linha chamava
    # auto_vincular_tecnicos_recentes(supervisor, mes_limite), que criava
    # o vínculo sozinha usando o supervisor_atual da base de visitas.
    # A função continua definida em repositorio_vinculo_tecnico.py, só não
    # é mais chamada — dá pra religar aqui se algum dia precisar de novo.
...
def tela_associar_tecnicos(request: Request, supervisor: str | None = Query(default=None)):
    ...
    tecnicos_ja_vinculados = set()
    if supervisor:
        vinculos_atuais = repositorio_vinculo_tecnico.listar_vinculos_do_supervisor(supervisor)
        tecnicos_ja_vinculados = {
            repositorio.normalizar(v["tecnico"]) for v in vinculos_atuais if v["data_desvinculacao"] is None
        }
    ...
    return templates.TemplateResponse(
        "associar_tecnicos.html",
        {
            ...
            "tecnicos_ja_vinculados": tecnicos_ja_vinculados,
        },
    )
    ...
    return templates.TemplateResponse(
        "cadastros_coordenador.html",
        {
            ...
            "vinculos_ativos_por_supervisor": {
                s["id"]: [
                    v for v in repositorio_vinculo_tecnico.listar_vinculos_do_supervisor(s["nome"])
...
def alterar_ativo_tecnico(request: Request, id_tecnico: int, ativo: bool = Form(...)):
    if not supervisor_logado(request):
        return RedirectResponse("/login", status_code=303)
    if request.session.get("tipo") != "coordenador":
        raise HTTPException(status_code=403, detail="Acesso restrito ao coordenador geral.")

    repositorio.definir_ativo_tecnico(id_tecnico, ativo)
    return RedirectResponse("/coordenador/cadastros", status_code=303)
